...
public class TasteAnalysisUtil {
    ...
    // 취향 유형 이름 매핑
    private static final Map<String, String> TASTE_TYPE_NAMES = Map.ofEntries(
            Map.entry("paef", "방구석 힐링 요정"),
            Map.entry("paeF", "감성에 잠긴 철학자"),
            Map.entry("paEf", "트렌드세터 몽상가"),
            Map.entry("paEF", "고독한 예술가"),
            Map.entry("pAef", "효율 추구 스마트 유저"),
            Map.entry("pAeF", "지적인 탐구 생활자"),
            Map.entry("pAEf", "방구석 크리에이터"),
            Map.entry("pAEF", "집념의 장인"),
            Map.entry("Paef", "자유로운 영혼의 나그네"),
            Map.entry("PaeF", "자연 속의 사색가"),
            Map.entry("PaEf", "에너지 넘치는 인싸"),
            Map.entry("PaEF", "열정적인 무대 체질"),
            Map.entry("PAef", "발로 뛰는 정보통"),
            Map.entry("PAeF", "끈기 있는 개척자"),
            Map.entry("PAEf", "감각적인 행동대장"),
            Map.entry("PAEF", "육각형 갓생 러너")
    );

    // boomLevel 가중치를 적용한 PAEF 점수 계산
    ...
    public static String determineTasteType(TasteScore score) {
        ...
        type.append(score.p >= threshold ? "P" : "p");
        type.append(score.a >= threshold ? "A" : "a");
        type.append(score.e >= threshold ? "E" : "e");
        type.append(score.f >= threshold ? "F" : "f");
    ...
    public static String getTasteTypeName(String tasteType) {
        return TASTE_TYPE_NAMES.getOrDefault(tasteType, "미분류");
    }
    ...
    public static double calculateEuclideanSimilarity(TasteScore user1, TasteScore user2) {
        ...
        // 2. 유클리드 거리 계산
        double distance = Math.sqrt(diffP + diffA + diffE + diffF);
